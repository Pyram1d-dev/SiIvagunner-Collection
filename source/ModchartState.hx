...
class ModchartState
{
	...
	function getBackground(spritePath:String, toBeCalled:String, drawBehind:Bool)
	{
		#if sys
		...
		// No fuck you. Better layering.
		if (drawBehind)
			PlayState.instance.addBehind(sprite, PlayState.gf);
		else
			PlayState.instance.addObject(sprite);
		#end
	...
	public function setDefaultStrumPos()
	{
		for (receptor in receptors)
		{
			receptor.defaultX = receptor.sprite.x;
			receptor.defaultY = receptor.sprite.y;
		}
	}
	...
	// LUA SHIT
	var receptors:Array<LuaReceptor> = [];
	...
	function new(?isStoryMode = true)
	{
		...
		var path = Paths.lua('SONGS/${songLowercase}/lua/modchart');
		...
		setVar("color", FlxG.save.data.color);
		...
		Lua_helper.add_callback(lua, "lerp", function(a:Float, b:Float, alpha:Float)
		{
			return CoolUtil.coolLerp(a, b, alpha);
		});

		Lua_helper.add_callback(lua, "playSound", function(path:String, ?directory:String)
		{
			FlxG.sound.play(Paths.sound(path, directory));
		});

		// Lua random is dogshit because randomseed doesn't work for some reason lol
		Lua_helper.add_callback(lua, "random", function(a:Float, b:Float)
		{
			return FlxG.random.float(a, b);
		});

		Lua_helper.add_callback(lua, "randomInt", function(a:Int, b:Int)
		{
			return FlxG.random.int(a, b);
		});

		for (i in 0...PlayState.strumLineNotes.length)
		{
			var member = PlayState.strumLineNotes.members[i];
			var receptor = new LuaReceptor(member, "receptor_" + i);
			receptors.push(receptor);
			receptor.Register(lua);
		}

		new LuaGame().Register(lua);

		new LuaStage().Register(lua);
