...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function goToSection(section:Int)
	{
		...
		curSection = section;
		trace("Going to " + FlxG.sound.music.time + " | " + section + " | Which is at " + beat);
	...
	override function update(elapsed:Float)
	{
		...
		var isMine = FlxG.keys.pressed.SHIFT;
		...
		for (p in 0...pressArray.length)
		{
			...
			if (i && !delete)
			{
					addNote(new Note(Conductor.songPosition, p, null, false, true), isMine);
			}
		}
	...
	private function addNote(?n:Note, isMine:Bool = false):Void
	{
		...
		if (n != null)
			section.sectionNotes.push([n.strumTime, n.noteData + (isMine ? 8 : 0), n.sustainLength, false]);
		else
		...
		else
		{
			...
			note.rawNoteData = n.noteData + (isMine ? 8 : 0);
			note.sustainLength = noteSus;
			note.setGraphicSize(Math.floor(GRID_SIZE), Math.floor(GRID_SIZE));
			note.updateHitbox();
	...
	{
		var difficultyArray:Array<String> = ["-easy", "", "-hard", "-extra"];
