...
class Paths
{
	...
	static public function sound(key:String, ?library:String, ?debug:Bool)
	{
		return getPath('sounds/$key.$SOUND_EXT', SOUND, library, debug);
	}
	...
	inline static public function image(key:String, ?library:String, ?debug:Bool = false)
	{
		...
		return getPath('images/$key.png', IMAGE, library, debug);
	}
	...
	inline static public function getSparrowAtlas(key:String, ?library:String, ?isCharacter:Bool = false, ?debug:Bool = false)
	{
		...
		return FlxAtlasFrames.fromSparrow(image(key, library, debug), file('images/$key.xml', library));
	}
