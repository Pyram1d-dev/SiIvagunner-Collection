...
class KeyBindMenu extends FlxSubState
{
	...
	{	
        ...
        infoText = new FlxText(-10, 580, 1280, 'Current Mode: ${KeyBinds.gamepad ? 'GAMEPAD' : 'KEYBOARD'}. Press TAB to switch\n(${KeyBinds.gamepad ? 'RIGHT Trigger' : 'Escape'} to save, ${KeyBinds.gamepad ? 'LEFT Trigger' : 'Backspace'} to reset keybinds to defaults and leave.${KeyBinds.gamepad ? ' START To change a keybind' : ''})', 72);
        ...
        //OptionsMenu.instance.acceptInput = false; // Had to change this for quick options to work. Luckily now this menu works wherever you want meaning you don't have to navigate back to the options menu every time.
	...
	override function update(elapsed:Float)
	{
		...
		infoText.text = 'Current Mode: ${KeyBinds.gamepad ? 'GAMEPAD' : 'KEYBOARD'}. Press TAB to switch\n(${KeyBinds.gamepad ? 'RIGHT Trigger' : 'Escape'} to save, ${KeyBinds.gamepad ? 'LEFT Trigger' : 'Backspace'} to reset keybinds to defaults and leave.${KeyBinds.gamepad ? ' START To change a keybind' : ''})\n${lastKey != "" ? lastKey + " is blacklisted!" : ""}';
