	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		if (curCharacter.startsWith('bf') && !exception)
		{
			if (!curCharacter.startsWith('bf-pixel') && !curCharacter.startsWith('bf-car') && !curCharacter.startsWith('bf-christmas'))
			{
				...
				loadOffsetFile((hasUniqueOffsets) ? baseCharacter : 'bf');
			...
			else
			{
						...
						case 'christmas':
							{
								...
								loadOffsetFile((hasUniqueOffsets) ? baseCharacter : (nameSplit[0] + '-' + nameSplit[1]));
				...
				else
				{
					...
					loadOffsetFile((hasUniqueOffsets) ? baseCharacter : 'bf-pixel-dead');
					playAnim('firstDeath');
					// pixel bullshit
					setGraphicSize(Std.int(width * 6));
					updateHitbox();
