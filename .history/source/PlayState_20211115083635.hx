...
class PlayState extends MusicBeatState
{
	...
	// ONLY USED FOR THE FUCKING COW LMAO
	// Ok but it could be useful in some other applications ig
	#if sys
	...
	override public function create()
	{
		...
		var rootExceptions = ['lo-fight', 'winter-horrorland'];

		var rootSong = (!rootExceptions.contains(songLowercase)) ? songLowercase.split('-')[0] : songLowercase;

		for (i in rootExceptions)
			if (songLowercase.startsWith(i))
				rootSong = i;

		var songsWithDialogue = ['senpai', 'roses', 'thorns', 'lo-fight', 'overhead', 'ballistic', 'wocky', 'beathoven'];
		...
		{
			songCache = new GameCache();
			songCache.loadFrames(['BEANED' => ['characters/Monsters/monsterChristmas_bean', 'shared']], false);
		}
		#end
		...
		var exceptions:Array<String> = ['monster-christmas', 'senpai-angry'];
		...
		switch (vibeCheck)
		{
			...
			case 'monster-christmas':
				dad.y += 130;
			case 'dad':
				dad.visible = (SONG.song != 'Fresh Itch io Build' && SONG.song != 'Test');
				camPos.x += 400;
		...
		stage = new Stage(stageCheck, rootSong, songLowercase, daPixelZoom); // sex
		if (!PlayStateChangeables.Optimize)
		{
		...
		var vibeCheck2 = (curStage.startsWith('school')) ? 'school' : curStage;
		...
		{
			if (SONG.stage == 'ballisticAlley' && !PlayStateChangeables.Optimize)
			{
				stage.swagBacks.get('wBg').alpha = 0;
				...
				funneEffect.setGraphicSize(Std.int(FlxG.width * 1.1), Std.int(FlxG.height * 1.1));
				funneEffect.updateHitbox();
				funneEffect.screenCenter();
				funneEffect.alpha = 0;
	...
	function OHGODNO()
	{
		...
		new FlxTimer().start(0.2, function(tmr:FlxTimer)
		{
			...
			FlxG.sound.play(Paths.sound('ohmygod', 'week1'));
			FlxG.sound.play(Paths.sound('Lights_Turn_On'));
	...
	function createHudObjects(updatePos:Bool = false)
	{
		if (!updatePos) // I dont wanna talk about this code :(   (Jarvis, insert spongebob crying image with vine boom sound effect bass boosted)
		{
			...
			add(iconP1);
			iconP2 = new HealthIcon((SONG.player2 == 'gf') ? gf.curCharacter : (SONG.song == 'Fresh Itch io Build') ? 'crazybus' : (SONG.song == 'Test') ? 'bf-pixel' : SONG.player2, false);
	...
	function tankmanIntro()
	{
		...
		var tankmanFinish = function()
		{
			...
			FlxTween.tween(camGame, {zoom: defaultCamZoom}, 1, {
				ease: FlxEase.quadInOut
			});
		}
		camZooming = false;
			...
			case 'ugh':
				{
					...
					if (FlxG.random.bool(75) && false)
					{
						add(blacc);
						var video:MP4Handler = new MP4Handler();
						video.playMP4(Paths.video('ughCutscene'), tankmanFinish);
			...
			case 'ugh-alterneeyyytive-mix':
				{
					...
					beep.onComplete = function(){boyfriend.playAnim('idle', true);}
					var talking = true;
					var ended = false;
					var part2 = function()
					{
					...
					/*new FlxTimer().start(.25, function(tmr:FlxTimer)
					{
						if (talking)
						{
							var randAnim:Array<String> = ['singLEFT', 'singRIGHT', 'singDOWN', 'singUP', 'singUP-alt'];
							if (randAnim.contains(dad.animation.curAnim.name))
								randAnim.remove(dad.animation.curAnim.name);
							dad.playAnim(randAnim[FlxG.random.int(0, randAnim.length - 1)], true);
						}
						if (!ended)
							tmr.reset(.25);
						else
			...
			case 'guns-short-version':
				{
					...
					var video:MP4Handler = new MP4Handler();
					video.playMP4(Paths.video('gunsCutscene'), tankmanFinish);
					new FlxTimer().start(1, function(tmr:FlxTimer)
					{
						remove(blacc);
					});
	...
	function whittyAnimation(?dialogueBox:DialogueBox):Void // Mfw I steal Whitty code :troll:
	{
		...
		switch (curSong.toLowerCase()) // WHITTY ANIMATION CODE LMAOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
		{
			case 'ballistic':
				...
				funneEffect = new FlxSprite(-600, -200).loadGraphic(Paths.image('thefunnyeffect', 'bonusWeek'));
				funneEffect.width *= 0.5;
				funneEffect.height *= 0.5;
				funneEffect.alpha = 0;
				funneEffect.scrollFactor.set();
				funneEffect.visible = true;
				add(funneEffect);
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var rootSong = StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase().split("-")[0];
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			if (SONG.noteStyle == 'pixel' || SONG.noteStyle == 'clubpenguin')
			{
	...
	function startSong():Void
	{
		...
		if (stage.curStage == 'militaryzone' || stage.curStage == 'homedepot')
			stage.swagBacks['rollingTank'].visible = true;
		...
		if (notes.members.length > 0 && notes.members[0].strumTime > 5000)
		{
			skipIntroText = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 150 : -150), 0,
				"Press SPACE to skip intro (Backspace to ignore)");
		...
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				case 'pixel' | 'clubpenguin':
					var ANOTHERFUCKINGADDER = (noteTypeCheck == 'clubpenguin') ? '/clubpenguin' : '';
					babyArrow.loadGraphic(Paths.image('weeb$ANOTHERFUCKINGADDER/pixelUI/arrows-pixels', 'week6'), true, 17, 17);
				...
				default:
					var sadge = (noteTypeCheck == 'kapi' ? '_kapi' : '');
					babyArrow.frames = Paths.getSparrowAtlas('NOTE_assets' + sadge);
					for (j in 0...4)
					{
	...
	function updateAA()
	{
		...
		if (SONG.noteStyle.toLowerCase() == 'pixel' || SONG.noteStyle.toLowerCase() == 'clubpenguin')
		{
	...
	{
		var bfColor:FlxColor = 0xFF0097C4;
		var dadColor:FlxColor = 0xFFFA0000;

		if (FlxG.save.data.color)
		{
			switch (SONG.player2.split('-')[0])
			{
	...
	override public function update(elapsed:Float)
	{
				...
				case 'ballistic':
					{
						...
						{
							if (curBeat >= 456 && curBeat < 488 && funneEffect.alpha < 1)
								funneEffect.alpha += 0.002 * elapsed * 144;
							if (curBeat >= 488 && funneEffect.alpha > 0)
								funneEffect.alpha -= 0.02 * elapsed * 144;
						}
					}
			}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				daNote.y -= (daNote.isMine ? ((FlxG.save.data.downscroll) ? 195 : 60) : 0);
				if (!daNote.mustPress
					&& daNote.wasGoodHit
					&& !(songLowercase == 'dad-battle-in-game-mix' 
					&& !FlxG.save.data.playEnemy) 
					&& !daNote.isMine)
				{
				...
				if (daNote.mustPress && !daNote.modifiedByLua)
				{
					...
					daNote.x = playerStrums.members[Math.floor(Math.abs(daNote.noteData))].x - (daNote.isMine ? 43 : 0);
					if (!daNote.isSustainNote || daNote.isParent)
					...
					&& !daNote.modifiedByLua)
				{
					...
					daNote.x = strumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].x - (daNote.isMine ? 43 : 0);
					if (!daNote.isSustainNote || daNote.isParent)
	...
	function pressArrow(spr:StaticArrow, idCheck:Int, daNote:Note)
	{
		if (Math.abs(daNote.noteData) == idCheck)
		{
			...
			if (FlxG.save.data.noteSplats
				&& ((daNote.rating == 'sick' || !daNote.mustPress) && (!daNote.isSustainNote || daNote.isParent)) && !daNote.isMine)
			{
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					trace(poop);
					var lightsOff = StringTools.replace(PlayState.storyPlaylist[0], " ", "-").toLowerCase() == 'fresh' || StringTools.replace(PlayState.storyPlaylist[0], " ", "-").toLowerCase() == 'winter-horrorland-short-version';
					if (lightsOff)
					{
			...
			if (pressArray.contains(true) && generatedMusic)
			{
				...
				if (currentPlayer.holdTimer > Conductor.stepCrochet * 4 * 0.001
					&& (!holdArray.contains(true)
						...
						&& !(SONG.song == 'Dad Battle In-Game Mix' && FlxG.save.data.playEnemy))))
				{
	...
	override function beatHit()
	{
		...
		{
			case 'militaryzone' | 'homedepot':
				stage.swagGroup.get('tankmen').forEach(function(dancer:Dancer)
				{
